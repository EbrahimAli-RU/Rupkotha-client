...
const Account = (props) => {
  ...
  const [exireIn, setExpireIn] = useState("");
  ...
  useEffect(() => {
    axios
      .get(`/payment/premium-user/${jwt_decode(token).id}`)
      .then((res) => {
        setExpireIn(res.data.data.payment.expireIn);
        if (
          res.data.data.payment.expireIn
            .split("-")[1]
            .toString()
            .substr(0, 1) === "0"
        ) {
          let month = res.data.data.payment.expireIn
            .split("-")[1]
            .toString()
            .substr(1, 1);
          setMonth(month);
        } else {
          let month = res.data.data.payment.expireIn.split("-")[1];
          setMonth(month);
        }
        setFlag(true);
      })
  ...
  const reloginHandler = () => {
    setLoading(true);
    axios
      .post(`/user/signin`, { email: user.email, password: password })
      .then((res) => {
        setLoading(false);
        setShow(false);
  ...
  console.log(monthList[9]);
  return (
    <>
      <Navigation />
      <Subscription
        show={subscriptionPackage}
        handler={closeSubscriptionPackageHandler}
      />
      <Error
        show={error.error}
        message={error.message}
        handler={vanisErrorHandler}
      />

      <div
        className={
          !showCuppon && !subscriptionPackage ? "blockDisplay" : "noneDisplay"
        }
      >
        <Modal show={cancelReward}>
          <div style={{ backgroundColor: "white", width: "42rem" }}>
            <h3 style={{ padding: "3rem 0rem 0rem 1rem", color: "black" }}>
              Cancel Subscription
            </h3>
            <p className="popup__question" style={{ fontSize: "1.6rem" }}>
              Are you sure? Your subscription will finish when the period ends
            </p>
            <div className="popup__button">
              <button onClick={closeModal} className="popup__btn">
                Cancel
              </button>
              <button style={{ color: "red" }} className="popup__btn">
                Confirm
              </button>
            </div>
          </div>
        ...
        <Modal show={show}>
          <div className="model__content">
            ...
            <Button
              ...
              loading={loading}
            />
            <Link className="cancel__button display" to="/home">
              Cancel
            </Link>
          </div>
        </Modal>
        {flag ? (
          <>
            <div
              className="create__profile__wrapper"
              style={{ marginTop: "10rem" }}
            >
              <div className="create__profile__content">
                <h2
                  style={{
                    fontFamily: "Lobster, sans-sarif",
                    fontSize: "3.5rem",
                  }}
                >
                  Account
                ...
                <AccountInput
                  ...
                  date={`Expires ${monthList[parseInt(month)]} ${exireIn
                    .split("-")[2]
                    .substr(0, 2)}, 
                            ${exireIn.split("-")[0]}`}
                  toShow
                  handler={showSubscriptionPackageHandler}
                />
                <AccountInput
                  width={true}
                  link="/"
                  title="Cancel renewal"
                  handler={cancelRewardHandler}
                />
