...
const Main = () => {
  ...
  const { loading, hasMore, book } = Books(
    page,
    wishlist.id,
    wishlist.category,
    wishlist.isInserted
    ...
    axios
      ...
      .then((res) => {
        res.data.data.carosul = res.data.data.carosul.map((el) => {
          return {
            ...el,
            isWishlisted: wishlistFromRedux.wishlist.findIndex(
  ...
  return (
    <div id="scroll">
      ...
      />
      <Navigation />
      {carosulItem.length === 0 && book.length === 0 ? (
        <Spinner
          className="center__main__spinner"
          animation="border"
          variant="info"
          variant="dark"
        />
        ...
        <>
          {carosulItem.length === 0 ? (
            <DotSpinner />
          ) : (
            <Carousel
              responsive={responsiveCarosul}
              autoPlay
              showDots
              infinite
              autoPlaySpeed={3000}
            ...
            >
              {carosulItem.map((el, i) => (
                <HomePageCarosul
                  ...
                  bookPhoto={el.bookPhoto}
                  backgroundPhoto={el.backgroundPhoto}
                />
              ))}
            </Carousel>
          )}
                  ...
                  return (
                    <div className="main__books" key={i}>
                      <div className="main__books__category">
                        <p>{el._id}</p>
                        <a href={`/book/channel?_channel=${el._id}`}>
                          View More
                        </a>
                      </div>
                        ...
                        {el.books.length < 5 ? (
                          <>
                            {el.books.map((singleBook, i) => (
                              <BookCard
                                title={singleBook.shortDescription}
