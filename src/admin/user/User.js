...
const User = () => {
    ...
    const getUserHandler = (nPage = page, rPPage = rowsPerPage) => {
        axios.get(`/admin/user?page=${nPage}&limit=${rPPage}&s=${searchBy}`, {
    ...
    const resellerSearchHandler = (e) => {
        ...
        axios.get(`/admin/user?page=${page}&limit=${rowsPerPage}&s=${e.target.value}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        }).then(res => {
            setUsers(res.data.data.user)
            setNumberOfUsers(res.data.data.count)
        ...
        <>
            ...
            <Card>
                <div className={classes.root}>
                    <Typography className={classes.marginleft} variant='h5'>User</Typography>
